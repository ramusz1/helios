...
def X2Cube(img,B=[4, 4], skip = [4, 4], bandNumber=16):
    ...
    DataCube = out.reshape(M//B[0], N//B[1], np.prod(B))[:,:,:bandNumber]
    return DataCube
...
def success_overlap(gt_bb, result_bb, n_frame):
    ...
    return thresholds_overlap, success
...
def compile_results(gt, bboxes):
    ...
    for i in range(50):
        precision[i] = sum(new_distances < distance_thresholds[i]) / np.size(new_distances)
    dp_20 = precision[19]
    average_center_location_error =new_distances.mean()
    # what's the percentage of frame in which center displacement is inferior to given threshold? (OTB metric)

    thresholds_overlap, success=success_overlap(gt4, bboxes, l)
    # integrate over the thresholds
    auc = np.mean(success)
    return (distance_thresholds, precision), (thresholds_overlap, success),average_center_location_error, auc,dp_20
